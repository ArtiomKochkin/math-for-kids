...
function generateEquation() {
    let number1 = Math.floor(Math.random() * 13);
    let number2 = Math.floor(Math.random() * 13);
    let falseAnswer1 = Math.floor(Math.random() * 10);
    let falseAnswer2 = Math.floor(Math.random() * 10);
    let allAnswers = [];
    let switchAnswers = [];
    
    trueAnswer = eval(number1 - number2);
